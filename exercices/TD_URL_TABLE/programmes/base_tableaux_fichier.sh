...
echo "<html><body>" > ./tableaux/$fichier_tableau
echo "<h2>Tableau $basename :</h2>" >> ./tableaux/$fichier_tableau
echo "<br/>" >> ./tableaux/$fichier_tableau
echo "<table>" >> ./tableaux/$fichier_tableau
...
while read -r URL; do
	...
	aspiration=$(curl $URL > ./aspirations/pl_$lineno.html)
...
echo "$dump" > ./dumps_text/pl_$lineno.txt
	
	fichierDump=./dumps_text/pl_$lineno.txt
	...
	echo "<tr><td>$lineno</td><td>$code</td><td><a href=\"$URL\">$URL</a></td><td>$charset</td><td>$compte</td></tr>" >> ./tableaux/$fichier_tableau
	echo -e "\t--------------------------------"
	lineno=$((lineno+1));
	
done < ./URLs/$fichier_urls
echo "</table>" >> ./tableaux/$fichier_tableau
echo "</body></html>" >> ./tableaux/$fichier_tableau
#aspiration html
#dumps texte textuel 
